...
| `circuit-reports.md` | `CircuitDiagnostics` | What a circuit actually is, per script: the counting table, the hydraulic partition, what answers each constraint, the Newton trajectory iteration by iteration, every unknown seeded (with its seed basis) against solved, every node in °C and kPa and every branch against its written direction, the heat balance per hydraulic, each pump's and valve's operating point, every residual, what sizing chose, exchanger ratings, and rank and conditioning. Covers `samples/` plus anything dropped in `scratch/` |
