...
| `layout/scratch-<name>.txt`, `.svg` | `LayoutDiagnostics` | Every `diagnostics/scratch/*.fluid` drawn as `28` §31 text and as SVG, beside the samples in `layout/` -- the layout half of `CircuitDiagnostics`, so one variant gets its solve report and its picture from one file |
...
| `pipeline-timings.md` | `PipelineTimingDiagnostics` | Where the time goes between a script and a solved circuit — parse, bind, lower and solve per sample, and inside one Newton step the cost of a residual evaluation, the finite-difference Jacobian sweep as `NewtonSolver` builds it (one column per unknown, re-fixing only the node it moves) beside the naive `N+1` residuals for comparison, and the dense LU of the same order |
