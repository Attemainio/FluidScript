...
TE1 p_sensor at N2                      # what the model produced
```
...
```fluidscript
control TV1 with TE1 by PID1 setpoint=312
```

Just `TE1`. A pressure sensor measures one thing, so there is nothing to disambiguate. Writing
`TE1.p` is also fine.
