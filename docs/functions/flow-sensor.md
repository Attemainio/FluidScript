...
TE1 flow_sensor at N2                      # what the model produced
```
...
```fluidscript
control TV1 with TE1 by PID1 setpoint=0.24
```

Just `TE1`. A mass flow sensor measures one thing, so there is nothing to disambiguate. Writing
`TE1.flow` is also fine.

Its reading is in kg/s.
