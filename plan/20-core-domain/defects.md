...
| C-62 | [`24`](24-auto-sizing.md), `D-88` | **The two-way valve rule rounds `kv` down unconditionally, which is only safe on a pump-driven circuit** | "Round **down** to the nearest catalogue Kv --- an undersized valve has more authority, which is the safe direction" holds where a pump absorbs the extra drop. On a **pressure-bounded** circuit --- both ends stating a pressure, which is every district-heating primary and the `m2-cooling-loop` primary --- it is false: a Kv below what the pressure balance requires cannot pass the design flow at *any* position, so rounding down makes the design point unreachable and the duty is silently not met. Found while writing the three-way rule, which needed the same distinction and states it. | Open, and **re-scoped after audit: the code is further along than this entry, and the plan is now behind the code.** `ValveSizer.Size` already implements the split --- `bounded ? catalog.SmallestSatisfying(...) : catalog.NearestBelow(...)` --- and `ValveSizer.CanSize` answers for `Valve` and `ThreeWayValve` alike, so the arithmetic is in place for both. What decides which branch runs is `SizingContext.AvailableDrop`, and **that is set at exactly one site**, `OuterLoop.cs:539`, inside the three-way pass. An ordinary two-way `valve` is therefore never told what the boundaries offer, `bounded` is always false, and it does round down unconditionally exactly as this entry says. So the fix is no longer a rule change: it is building the same context in the ordinary sizing pass, which is the `driven`/`Offered(graph)` pair already written a few lines above. **Three things have to move together and none has:** (1) that context, (2) `24`, which still reads "That rule rounds down unconditionally... Recorded here rather than edited into the two-way rule, because changing a settled rule needs its own decision (`C-62`)" --- a deferral the code did not wait for; and (3) `24`'s acceptance criterion, "the achieved authority after rounding Kv down is **greater** than the target, never less", which is **already false** for a three-way valve on a bounded circuit and has been since the split shipped. There is still no `D-` entry for the rounding direction, which is what this entry asked for and the part genuinely outstanding. `24`'s own Provenance note remains the honest bit: the survey found the authority definition stated repeatedly and the catalogue-rounding direction stated **nowhere**, so neither direction is an inherited convention this project would be departing from. |
