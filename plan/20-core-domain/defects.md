...
| C-67 | [`22`](22-component-model.md) | **A heat exchanger can state `in=50 out=30 power=+24` --- fluid cooling, duty claiming heating --- and nothing says so** | `power` is positive when side 1 gains heat, and the docs are explicit that *"a cooler is written with a negative `power`, never a negative `dt`"*. Nothing checks the stated triple against itself. `in > out` means the fluid leaves colder than it arrived, so the duty **must** be negative; the two statements are contradictory and the component's own energy balance cannot satisfy both. `m2-distribution-header` carried it on both consumers and nothing reported it: the circuit read as open, so `FS2203`'s closed-circuit heat sum never ran, and the contradiction surfaced only as a residual of `-2 K` on each `out` statement, which reads like a convergence problem rather than a modelling error. `m1-syntax-tour` and `m1-syntax-reference` still carry the same shape on four exchangers. | Open. The check is arithmetic --- `sign(power) == sign(out - in)` whenever all three are stated --- and belongs beside the other stated-triple validation rather than in the solver, where it arrives as a residual nobody can attribute. It needs a diagnostic code and a decision on whether a stated `dt` participates. The `m1-*` samples need auditing at the same time; they are syntax tours rather than solvable circuits, which is why no counting or solve check has ever objected to them. |
...
| C-4 | [`22`](22-component-model.md) | `heat_exchanger` mode selection is unimplemented | Duty, Rated and Coupled are computed at lowering from connections and stated properties. The registry marks `in2`/`out2` optional, which is all the binder needs; the rest is P3.3/P4.1. **Still open after `P3.3`, and now sharper than "unimplemented": `HeatExchanger.Mode` returns `ExchangerMode.Duty` unconditionally** (`Valve.cs`-style hard-coding at `HeatExchanger.cs:145`), so the property reports `duty` for a Rated or Coupled exchanger and the enum's other two members have no producer. `WellPosedness.IsCoupled` decides coupling separately for counting, so the two answers can disagree without anything noticing. |
