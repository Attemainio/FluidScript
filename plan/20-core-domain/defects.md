...
|---|---|---|---|
| C-63 | [`24`](24-auto-sizing.md) | **A three-way valve's two legs are coupled, and sizing from either one alone gets the other wrong** | Both legs share one `kv` rating and one `position`, and their effective coefficients are complementary --- so a drop chosen for the controlled leg fixes the bypass leg's drop as well, whether or not that one is sensible. Measured on `m2-cooling-loop`: sizing the controlled leg for the 18.9 kPa the pressure balance leaves gives Kv 1.6, and the solver then has to close the valve to `position` 0.41 to satisfy the *other* leg, at which point the bypass's effective Kv is about 0.16 and it drops **316 kPa** passing 0.0766 kg/s --- which is where the 33.6 m pump head came from. Sizing instead for the bypass, via the authority target on the variable circuit's 2.26 kPa, gives Kv 4 and the controlled leg can then no longer restrict enough. | Open, and it is a **rule** gap rather than a plumbing one: `24`'s three-way rule sizes from one leg and says nothing about the other, so no implementation of it as written can be right on a mixing circuit. **Looked up, and the hypothesis held.** Industry guidance is unambiguous that a **balancing valve in the bypass leg is required**, set so the bypass drop matches the path it bypasses; without one "a fluid short-circuit occurs and the supply-to-return differential pressure in the system will drop". So the rule is in two parts and only the first is the valve's: size `kv` from the controlled leg, and put a matched balancing resistance in the bypass. One coefficient cannot satisfy two legs carrying different flows. **`m2-cooling-loop`'s bypass branch is empty** --- no components at all between the valve and the mixing node, so its resistance is exactly zero, which is that short-circuit. The fixture is missing a real component rather than the sizer missing a rule, so the next step is a change to the sample (and to `01`, which owns it) rather than more sizing code. `24` now states both parts with sources. **The sample could not carry that component until `S-35` was fixed**: adding anything at all to the bypass leg -- a `valve`, or a plain `pipe`, identically -- made the circuit `Singular` at iteration zero, because the seed gave both legs of the valve the same pressure and `position` became a column of zeros. That is now closed, and all four leg topologies reach `01`'s figures, so the fixture change is unblocked and what remains here is genuinely the sizing rule: `3WV.kv` is still the bootstrap 630 ("no rule sizes a three_way_valve's `kv`"), so `position` drives to its bound trying to compensate and the Kv law stays about 1.4 kg/s out. Blocks `m2-cooling-loop`, an **M2a** exit criterion. |
| C-62 | [`24`](24-auto-sizing.md) | **The two-way valve rule rounds `kv` down unconditionally, which is only safe on a pump-driven circuit** | "Round **down** to the nearest catalogue Kv --- an undersized valve has more authority, which is the safe direction" holds where a pump absorbs the extra drop. On a **pressure-bounded** circuit --- both ends stating a pressure, which is every district-heating primary and the `m2-cooling-loop` primary --- it is false: a Kv below what the pressure balance requires cannot pass the design flow at *any* position, so rounding down makes the design point unreachable and the duty is silently not met. Found while writing the three-way rule, which needed the same distinction and states it. | Open. It is a change to a settled rule and to an acceptance criterion ("the achieved authority after rounding Kv down is **greater** than the target, never less"), so it needs its own decision rather than an edit. Worth noting the industry survey behind the three-way rule found the authority definition and its bands stated repeatedly across manufacturer and engineering guidance, and the **catalogue-rounding direction stated nowhere** --- so neither direction is an inherited convention this project would be departing from. No sample is currently affected: `m2-simple-loop`'s `CV1` is on a pump-driven ring, where rounding down is correct. |
| C-61 | [`24`](24-auto-sizing.md) | **Nothing sizes a three-way valve wired as a three-way** (the two-way-wired case is closed --- see below) | The rule cannot simply be widened to the type. A three-way valve is a **junction element**, so it appears in no branch's `Path`, and `OuterLoop.Context` -- which finds a component's branch by `Path.Contains` -- can build it no context at all. It sits on three branches at once. That is the same structural limit `C-49` hit with parallel-set balancing, and it has the same answer: **a pass over `OuterLoop`**, which can see sibling branches, rather than an `ISizer`, which is handed one. What is missing beyond the plumbing is the *rule*: `24` says nothing about three-way valves, and a mixing valve's authority is measured against the variable-flow part of the circuit rather than against one branch -- so it needs to know which of `b` and `c` is the controlled path and which the bypass. **`24` now states the rule** (`### Three-way valve --- kv`), so what is left is the `OuterLoop` pass that implements it. The decision that had to be made first was not which port is controlled -- `22` already settles `a` common, `b` controlled, `c` bypass -- but that **the drop is chosen on a pump-driven circuit and determined on a pressure-bounded one**, and that the two round in opposite directions. **The two-way-wired case is now handled and was a category error rather than a gap.** Measured: on `m2-distribution-header` `TV_AHU` and `TV_RAD` have `BypassConnected` false, are **not** junction elements, and sit *inside* a branch's `Path` --- so `OuterLoop.Context` builds them exactly the single-branch context a `valve` gets, and only `CanSize`'s `component is Valve` excluded them. `22` already says a three-way used as a two-way "leaves [`c`] open, and inference rule I3 terminates it"; hydraulically it *is* a two-way valve. `CanSize` now reads `component is Valve or ThreeWayValve { BypassConnected: false }`. The header's valves still do not size --- their branch reports 0 kPa excluding the valve at the iterate the loop reaches, so the rule declines rather than sizing into the smoothing band --- but they now say so in a note naming the cause and the two ways out, where before they held Kv 630 with no basis at all. What remains open is the genuine three-way, and the pass for it was **built, measured twice and reverted**. The mechanics all worked: the legs are identified by mass balance rather than port letter (the common leg is the one whose flow is the sum of the other two, measured as 0.23926 = 0.076256 + 0.16301), the variable leg is the one reaching a stated pressure, and the design flow came out at 0.163 kg/s exactly as `24`'s worked example has it. Both sizing branches were then wrong, and in opposite directions. **Treating the circuit as pressure-bounded** gave Kv 1.6 --- `24`'s own worked answer --- and the sample *converged*, on every figure `01` states: mixing node 20.06 &deg;C, return 49.99 &deg;C, recirculation 0.0766 kg/s. It also gave `PU1` a head of **33.6 m**, about 330 kPa, on a loop whose exchanger drops 5 kPa. **Treating it as pump-driven** --- correct, because the primary path runs through a pump whose head nothing states, so the 20 kPa between the boundaries never bounded the valve --- gave Kv 4, and the sample stopped converging with the position driven to 0 and the head to 0. |
...
| C-51 | [`24`](24-auto-sizing.md), `D-58` | **No sizing rule reads a design driver other than `tout`, and the fraction-of-peak rule has nowhere to live** | `D-58` gives *each driver* a value and `D-59` resolves driver names through a registry, so `design tout=-26 draw=...` is already grammatical. But `24`'s rules all take a flow or a duty and none reaches `ProjectSettings.Design` at all, and the design point is a maximum load occurring a few hours a year. The dominant machine should not be sized there: a heat pump is normally sized to 50-70 % of peak with backup, because sizing to peak means cycling all winter at poor efficiency and costing far more. A DHW circuit's design condition is not an outdoor temperature at any fraction -- it is a draw profile. Deferred to `P3.8`, which is where sizing first reads the design point; recorded here so that package does not ship the single-driver reading and call it done. |
...
| C-48 | [`24`](24-auto-sizing.md) | **`pipe.velocity_min` is a catalogue row no rule reads, and at a 100 Pa/m target the sizer would select a pipe it knows trips its own validator** | The pipe rule checks the velocity *maximum* and steps up; nothing reads the 0.3 m/s minimum, which surfaces instead as `FS4005` from validation ([`01`](../00-foundation/01-vision-and-scope.md):556). Colebrook on EN 10255 bores for 20 kW at 70/40 (0.160 kg/s, water at 55 C) gives DN15 585 Pa/m, DN20 **132 Pa/m at 0.438 m/s**, DN25 **42 Pa/m at 0.276 m/s**. So a 150 Pa/m target selects DN20 and a 100 Pa/m target selects DN25 -- **at 0.276 m/s, under the sedimentation minimum**, with the step-down available and unconsidered. The whole 100-versus-150 argument is one catalogue step for this branch and there is nothing in between. Needs an explicit precedence: `velocity_max` hard, gradient a target, `velocity_min` a soft bound that steps down and reports. The deeper criterion is the index circuit's head budget, which a per-pipe local rule cannot see; that belongs with `FS2303`. |
...
**The port letters are positional, so `b` is not reliably the controlled path.** `22` names `a` common,
`b` controlled, `c` bypass, and the three-way sizing rule was written on the assumption that `b` carries
the variable flow. Measured on `m2-cooling-loop`, it does not: port `b` binds to `N2` and carries the
**recirculation** at 0.076 kg/s while port `c` binds toward `N3` and carries the **primary draw** at
0.163 kg/s. The binding follows the order the connections were written --- `3WV - N2` before
