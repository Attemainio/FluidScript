...
| C-40 | [`22`](22-component-model.md), [`23`](23-topology-and-graph.md) | **A heat exchanger and the node it discharges into both claim the same enthalpy relation** | The exchanger declares `Q = ṁ(h_out − h_in)` and the downstream node's energy balance, written over its ports with the arriving enthalpy, reduces to `h_own = h_arriving` for a degree-two node — the same relation with `Q` missing. Both rows land in the assembled system, so it is over-specified by exactly one per exchanger, while `23`'s counting table counts neither the duty row nor the conflict: `Relations` counts a crossed component once as a pressure relation and `EnergyBalances` is `Nodes.Length` unconditionally. The totals therefore agree with the document and disagree with the components. Owned from the solver side as [`S-11`](../30-solver/defects.md), where the assembler that has to emit or drop the row lives; recorded here because the fix is most likely a structural flag on `CircuitNode` — the mass balance is already conditional and decided at lowering in exactly this way. |
