...
| C-26 | [`23`](23-topology-and-graph.md) | **The counting table gives a flux unknown to a stated `p` but counts a stated `flow` as an equation with no unknown**, which over-specifies every terminal that states one | `23`'s unknown column has "external mass flux, one per node with a stated `p`"; its equation column has "stated `flow` boundaries" and no matching unknown. The storage header states four flows and one pressure, and under that scheme counts 17 equations against 13 unknowns — over-specified by four, on a reference circuit `01` gives derivative values for. What is symmetric is that a stated `flow` makes the flux *known*: it is a coefficient in the node's mass balance, exactly as a stated `t` is a coefficient in its energy balance, and it adds neither a row nor an unknown. P3.4b implements that reading and the header counts 13 = 13. Open because `23`'s table still says otherwise. |
| C-27 | [`23`](23-topology-and-graph.md) | **The pressure datum and the redundant mass balance are described as one mechanism and are two** | "Exactly one of the two mechanisms applies, never both and never neither." That holds for the circuits `23` tabulates and fails on one whose only stated pressure sits on a node with no mass balance — a degree-two node interior to a branch, where nothing can accept external mass. The stated pressure then fixes the pressure *level*, so no datum equation is needed, but the component still has no unknown flux, so a mass balance is still redundant. Conflating them counted the distribution header one equation high. The two conditions P3.4b implements are: add a datum when no pressure is stated at all; drop a balance when no *balancing* node states one. |
