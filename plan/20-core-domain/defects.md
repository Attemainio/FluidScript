...
| C-67 | [`22`](22-component-model.md), `D-91` | **A neutral heat exchanger can state `in=50 out=30 power=+24` --- fluid cooling, duty claiming heating --- and nothing says so** | `power` is positive when side 1 gains heat. Nothing checks a neutral `heat_exchanger`'s stated triple against itself. `in > out` means the fluid leaves colder than it arrived, so its explicitly signed duty **must** be negative; the two statements are contradictory and the component's own energy balance cannot satisfy both. `m2-distribution-header` carried this shape on both consumers and the contradiction surfaced only as a convergence residual. `D-91` fixes the user-facing case without weakening the core convention: `HE_AHU load power=24 in=50 out=30` now lowers to `-24 kW`, and the sample uses that spelling. | Open, but no longer a blocker for ordinary loads. The remaining check is arithmetic for the deliberately neutral spellings --- `sign(power) == sign(out - in)` whenever all three are stated --- and belongs beside the other stated-triple validation rather than in the solver. It still needs a diagnostic code. A stated `dt` does not participate: `dt` is a positive magnitude by `D-91`, so it cannot contradict direction on its own. The `m1-*` syntax samples still need auditing; they are not solvable circuits, which is why no counting or solve check has objected to them. |
