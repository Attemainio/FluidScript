...
|---|---|---|---|
| C-66 | [`24`](24-auto-sizing.md) | **The three-way sizing pass identifies the variable leg by looking one hop, so no valve on a header is ever sized** | `OuterLoop.Reaches` asks whether the element at the far end of a leg states a pressure. On `m2-cooling-loop` that works, because the valve's controlled leg runs straight into `N3 return p=280`. A header states its pressure at the plant, three or more hops from any valve leg, so **every** three-way valve on it declines with "neither of the legs it controls reaches a stated pressure" and keeps bootstrap Kv 630. Measured across five source arrangements: all three valves decline in every one. The criterion is a proxy for the thing that actually matters --- which leg leaves the valve's own loop --- and it only coincides with it when the plant boundary happens to be adjacent. | Open. The structural reading is that **the bypass leg is the one that closes the valve's own loop**, so it is the leg whose far end reaches the common leg's far end in fewer components without passing back through the valve; the other leg is the variable one. Checked by hand on both fixtures: on `m2-cooling-loop` the recirculation leg reaches `HE1` in two hops and the controlled leg cannot reach it at all (it leaves through a boundary), and on the header the recirculation leg reaches `HE_AHU` in two hops against roughly eight the other way. **This is this project's reasoning, not an inherited convention**, and the case it would get wrong is a short tap off a header feeding a long secondary --- worth a test before it is trusted. A stronger signal exists and was not needed: on the header the two legs land in *different circuits*, which the script declares outright. Sizing the valve does **not** on its own fix the header, because `position` stays promoted either way (`S-38`). |
