...
| C-43 | [`22`](22-component-model.md) | **`IFlowComponent.DeclareUnknowns` has no consumer, and the one kind that uses it is modelled by nothing** | `22` puts `DeclareUnknowns` on the interface so a component can add scalars the solver varies, and it says `Index` is "filled in by assembly". `P3.6a` built both halves of that assembly and neither calls it. Only the tank declares anything — one enthalpy unknown, `T1.h`, alongside one energy balance — and the counting table counts neither, because `EnergyBalances` is `Nodes.Length` and the unknown side has no term for a component-owned scalar. The two omissions cancel exactly, so `m4-storage-header` reports square and is not. Recorded from the solver side as `S-16`, where the decision belongs: either a control volume owns an unknown and a row the table must model, or `D-69`'s flux treatment reaches the tank as well. Until then a computed allowance in the tests names it on every sample. |
