...
| C-30 | [`23`](23-topology-and-graph.md) | **`FS2210` fires on any closed circuit that states a temperature**, which is most of them — resolved, and recorded because of how it was found | Not open: fixed in `P3.4c` and moved to Closed below. Kept as a pointer because the *route* to it matters. The count was checked against `23`'s own worked example and balanced at 20 = 20 on the first run, against a hand-tabulated table, on a circuit that could not exercise the bug — the cooling loop is open. It took the user asking for a reference circuit to be *fixed* to reach a closed one, and the fix not working. A worked example is a regression test for the case it was written from and evidence about nothing else. |
