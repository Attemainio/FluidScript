...
tier: 50-frontend
status: draft
owns: [SVG canvas rendering, viewport, layout engine, routing, declarative-symbol interpretation, axes]
depends_on: [25-layout-hints, 26-model-contract, 51-frontend-architecture]
traces_to: [R-22, R-23, R-27, R-34, R-37, R-41, R-42, R-44, R-45]
open_questions: 0
last_review_pass: 0
---
...
### Approach: thermal bands, then layered with loop special-casing
...
2. Partition   Within each band/group, split into loop members (hints.loops) and tree parts.
3. Loops       Lay each loop out as a rectangle in hints.loopOrientations, components distributed
               around its perimeter in traversal order, sized by member count. Shared-component
               loops sit side-by-side with that component between them. This is the step a generic
               algorithm gets wrong, and it is what makes a circuit look like a circuit.
4. Tree parts  Layered locally by hints.rank: rank = column, siblings stack vertically.
5. Attach      Tree parts attach to their loop at the shared node's perimeter position.
6. Compact     Remove empty local columns/rows without crossing a thermal-band boundary; centre.
7. Route       Orthogonal routing between port anchors (below), including right-to-left returns.
```
...
| `controller` | M4 | Dashed circle with the loop tag, connected to its actuator by a dashed line |
...
- **Labels**: the component name always; key values (duty, DN, Kv) at zoom above 1.5×, hidden below to
  avoid clutter.
