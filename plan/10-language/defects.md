...
| L-45 | [`12`](12-grammar.md) | **`kind-name = identifier` was load-bearing reasoning and is no longer true** | `12` argued that `node` and `pipe` could be unreserved because a kind name "cannot match a keyword token" — using the production as evidence for the safety of the change. `D-64` puts `supply` and `return` in kind position, so the production is now `identifier | keyword` and the argument that carried it has to be the real one: **position** disambiguates, and always did. A statement whose first token is reserved is that word's statement; a declaration's first token is the name the user chose. `12` now says that instead, and the parser accepts a keyword in second position and hands the spelling to the registry. Open only because the *reverse* direction is unspecified: nothing says what happens if a future reserved word is added that a registry alias already claims, and `ComponentRegistry.Verify` currently bans a reserved word as an alias while allowing it as a kind's own keyword — a rule that exists in code and in no document. |
...
| L-44 | [`15`](15-semantic-model.md) | **The binder's step order put attachment resolution after inference, and `23` requires it before** | `supply N3` lowers to a connection from the parent's `N3` to the subcircuit's first unconnected inlet. Run after I3, there are no unconnected inlets left: the rule has already terminated every one of them with a dead-leg node. P3.4b moved `BindAttachments` to before `ApplyInference` and had it emit the two connections, which is also why the distribution header's four I3 nodes no longer exist. `15`'s step list still shows attachments at step 9. Two consequences worth stating: the `_sourceConnections` snapshot now includes attachment-derived connections, which is right because `supply` *is* a connection the user wrote; and `ReportDeadEnds`'s exemption for attachment parent nodes is now dead code, since the connection it stood in for exists. |
