...
| P6.11 | **FluidScript 2** (`D-164`–`D-169`, [`19`](10-language/19-fluidscript-2.md), added 2026-09-25): a second language major beside the first, selected by the version line, with its own lexer, parser, syntax tree and printer and a translation step into the shared binder. Seven packages, each closed with its docs, tests and register rows: **1** the spec (`19`, the decisions) -- written 2026-09-25 and stopped for the user's review; **2** lexer, parser and printer, the round-trip fuzz test on language 2; **3** the translation and what the binder newly learns (a per-case `let`, a curve of a `let`, the controller declaration, runs), port inference by flow direction; **4** the audit of all 186 diagnostic codes against language 2 and the `FS18xx` range; **5** the frontend editor -- tokenizer and completion chosen by the version line, hover, go-to-definition, the new-file template; **6** the tutorial written in language 2, a reference page per statement, the language 1 reference under `docs/v1/`, language 2 twins of the samples; **7** the switch -- language 2 becomes the current major, a decision of its own. **Runs before P6.3**, the user's call: P6.3 builds the controllers, and `D-168` changes how a controller is written. Exit: every sample's language 2 twin binds to the same model and the same solve as the original (`19` invariant 6) |
| P6.12 | **Property tables** (`D-173`, [`21`](20-core-domain/21-fluid-and-state.md) §Property tables, added 2026-09-25): the equation of state precomputed on a fixed lattice in (ln p, h), bicubic Hermite nodes carrying T, ρ, μ, k, c_p, s and their slopes, built lazily into a process-lifetime store with a memory cap and never written to disk, to a tolerance level (`standard` by default). Three packages: **T1** the store and liquid water -- the lattice, the node and cell, the levels, the cap and eviction, one build per tile under concurrency, a tabulated `ISubstance` in front of the exact one with its (p, T) inverse on the table, the run building its start's tiles before the first step, the report; **T2** the pure refrigerants -- liquid and vapour tables with metastable nodes one cell past the saturation line, the dome from one-dimensional saturation tables, CO₂ near its critical point measured; **T3** the blends -- (T, ρ) node placement with the phase imposed, the dome in (p, quality), per-tile refinement and its mixed-depth edges -- which waits on a blend being a substance at all and on `C-135`. **Runs after P6.11 and before P6.3**, the user's call: the tables are to exist before the transient work builds on the run. T2 is worth building only once a refrigerant circuit runs dynamically. Exit (T1): `21`'s table criteria -- the two transient samples on `standard` tables with their ms per step against the exact and constant-property runs, every frame inside the tolerance band, V8 agreeing, a rebuilt table bit-identical |
