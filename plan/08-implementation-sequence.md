...
| P5.13 | **Port-indexed state syntax** (`D-120`, added 2026-09-19): `in[n].quantity` on declarations and in references, brackets for every indexed port family, one property table with symbol and name shared by parameters, references and `show`, the old spellings read with a quick-fix suggestion, samples and `docs/functions/*` rewritten to the new form ([`12`](10-language/12-grammar.md), [`13`](10-language/13-type-and-unit-system.md), [`15`](10-language/15-semantic-model.md), [`22`](20-core-domain/22-component-model.md)) -- before P6, because M4's tank layers and controllers would otherwise be specified in a spelling about to change. Runs as two: **a** the syntax and the binder -- tokens, the qualified name, the registry's name/key split, the property table, the three codes, the corpus and the pages, the model below the binder untouched so nothing downstream moves while the spelling does; **b** what needs the model to move -- `in.p` on a component bound to the node its port touches (decision 3), `vflow` as a constraint evaluated at the port's state (`D-120`'s open question), and the keyed spellings the counting table, the sizes and the wire still show (`L-56`) |
| P5.14 | **The Core refactoring** ([`70`](70-core-refactoring.md), added 2026-09-21): the seams the architecture review named -- parameter ownership (`D-130`, `D-131`), the solved view, the hydrostatic and volume-flow helpers, the outer loop's refusal and annotation, the ring forms' rollback, the shared segment geometry -- each package behaviour-preserving and measured byte-identical on the corpus, the ladder and the goldens; R6 (the binder's phase records, `EquationSystem`'s builder) deferred to the feature that next opens those files -- after P5.13, because P5.13b had just moved the model under the binder and M4's controllers promote the actuators `D-130` orders, so both are specified against the seams and not against the code they replaced |
...
| P6.8 | **Sizing over scenarios** (`D-143`, 2026-09-22, superseding `D-138`): the `scenarios` declaration and array parameter values with `FS1540`–`FS1544` ([`12`](10-language/12-grammar.md), [`15`](10-language/15-semantic-model.md)); `design <name>` as the operating scenario alone; the four-step pipeline — solve each scenario sizing freely, merge parameter by parameter under the kind's rule, re-solve every scenario against the merged sizes frozen, draw the merged plant with `design`'s numbers; the governing scenario in every size's basis; the chunked workers and the sizing report ([`24`](20-core-domain/24-auto-sizing.md) §Sizing over scenarios). Tier-20 work independent of P6.0–P6.7 and runnable in parallel with them; placed here because it changes nothing a file without a `scenarios` line does and the transient reads its merged sizes from the snapshot. **Measure, do not assume:** that step 3's re-merge loop terminates on a plant whose valve, pipe and pump sizes move together; and whether the two mitigations `24` §What this gives up names are needed, since a hand-written scenario list sizes an interior peak to zero. **Shipped as P6.8a (2026-09-22, the language) and P6.8b (2026-09-23, the pipeline), sequentially rather than with the chunked workers** — the premise moved when `D-143` replaced the sweep with a hand-written list and `D-137` made a solve 7–17 ms, so six cases are ~100 ms; `24` keeps the chunked design and the report measures whether it is ever wanted. The first measurement is answered: the merge settles in **two rounds** on `m5-scenarios`, and the flow trap is real — the *smaller* duty governs every flow-driven size. The second is still open, with `C-121` in front of it: a merged plant reports no valve authority, so the minimum-flow row of the envelope table cannot be implemented yet |
| P6.9 | **The source structure** ([`71`](71-source-structure.md), `D-147`, added 2026-09-23): Core reorganised into nine domain folders whose namespaces mirror them, one type per file, concern partials for every class over ~600 lines (absorbing `70`'s R6 where it opens the same files), `ComponentBase` / `ValveComponentBase` / `SizerBase<TComponent>` and the pipe-catalogue builder — S0–S5, each behaviour-preserving and closed on every golden byte-identical. **Runs before P6.3**, the user's call: the transient packages open `Solvers/` and `Components/`, and doing them first would patch the flat layout once more. Measured before starting: about +1 700 lines net, because the restructure is for navigation and file headers outweigh the members the bases remove |
| P6.10 | **The layout engine rebuilt** (`D-153`, [`28`](20-core-domain/28-layout-solver.md) part E, added 2026-09-23): decompose, compose, draw, beside the old engine until parity. R0 the parity harness -- both engines on every ladder step and sample, a per-step report of audit counts and geometry difference, the new engine selectable behind `LayoutSolver`; R1 the circuit view (ports and peers, flows, runs, fragments); R2 the series-parallel decomposition and its trace; R3 occupancy, footprints and the one run-length calculation; R4 composition, rule by rule in ladder order (steps 1 → 11d), each step to parity or shown to the user; R5 the run builder, signals, crossings, labels, groups and scene; R6 the switch -- H11 hard in the audit, ladder steps 12 (three zones, `C-126`) and 12b (the zones with their instruments, `C-128`), the old engine deleted, `C-92`'s timing measured again. **Runs before P6.3**, the user's call: the three-zone plant, the commonest shape in heating, does not draw. Closes `C-126`, `C-127`, `C-128`; expected to decide `C-108`, `C-125` and `C-92` |
| P6.11 | **FluidScript 2** (`D-164`–`D-169`, [`19`](10-language/19-fluidscript-2.md), added 2026-09-25; **`D-174`: it replaces language 1**): the language that replaces language 1, selected by the version line, with its own lexer, parser, syntax tree and printer. Packages 1–4 build it beside language 1 through a translation into the shared binder: **1** the spec (`19`, the decisions); **2** lexer, parser and printer, the round-trip fuzz test on language 2; **3** the translation and what the binder newly learns (a per-case `let`, a curve of a `let`, the controller declaration, runs), port inference by flow direction; **4** the audit of the diagnostic codes against language 2 and the `FS18xx` range. Packages 5 onward replace language 1, in the order `D-174` rule 3 sets, nothing deleted before it is proven: **5** validate -- a language 1 → language 2 converter as a development tool, the whole corpus converted (`samples/`, the ladder, variants and stress files, the fenced blocks in `plan/` and `docs/`, the scripts inline in tests and the frontend), each script proven to bind to the same model and give the same solve through the translated path, and what cannot be converted listed for the user's decision; **6** bind directly -- the binder reads the language 2 tree, port inference, the controller, cases and runs move in from the translation, and on the converted corpus the direct path gives the translated path's model; **7** the switch -- the converted corpus replaces the original, goldens regenerate with nothing but spans changing, each code keeps one template (its language 2 wording), `SupportedVersions` becomes {2}, and the language 1 parser, the translation, the wording pass and the converter are deleted; **8** the frontend editor, language 2 only -- tokenizer, completion, hover, go-to-definition, canvas write-back, the new-file template, the run picker; **9** the documentation in language 2 -- the tutorial and a reference page per statement. **Runs before P6.12 and P6.3**, the user's call: no later package writes its tests in a language about to go. Exit: package 5's proof over the whole corpus, package 6's over the same corpus, and no language 1 code left in Core |
| P6.12 | **Property tables** (`D-173`, [`21`](20-core-domain/21-fluid-and-state.md) §Property tables, added 2026-09-25): the equation of state precomputed on a fixed lattice in (ln p, h), bicubic Hermite nodes carrying T, ρ, μ, k, c_p, s and their slopes, built lazily into a process-lifetime store with a memory cap and never written to disk, to a tolerance level (`standard` by default). Three packages: **T1** the store and liquid water -- the lattice, the node and cell, the levels, the cap and eviction, one build per tile under concurrency, a tabulated `ISubstance` in front of the exact one with its (p, T) inverse on the table, the run building its start's tiles before the first step, the report; **T2** the pure refrigerants -- liquid and vapour tables with metastable nodes one cell past the saturation line, the dome from one-dimensional saturation tables, CO₂ near its critical point measured; **T3** the blends -- (T, ρ) node placement with the phase imposed, the dome in (p, quality), per-tile refinement and its mixed-depth edges -- which waits on a blend being a substance at all and on `C-135`. **Runs after P6.11 and before P6.3**, the user's call: the tables are to exist before the transient work builds on the run, and P6.11's switch comes first (`D-174`). T2 is worth building only once a refrigerant circuit runs dynamically. Exit (T1): `21`'s table criteria -- the two transient samples on `standard` tables with their ms per step against the exact and constant-property runs, every frame inside the tolerance band, V8 agreeing, a rebuilt table bit-identical |
