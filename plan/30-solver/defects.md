...
|---|---|---|---|
| S-40 | [`32`](32-steady-state-newton.md), [`36`](36-numerics-and-convergence.md) | **`FS3010` reports nothing on a circuit that demonstrably has a row null direction** | The rebuilt `m2-distribution-header` terminates `Singular` and emits `FS3002` and `FS3009`, but **not** `FS3010`. The direction is there: a finite-difference Jacobian taken at the same point names seven rows, mass balances at weight 1, which is exactly what `S-39` was found from. `NullDirection.Redundancy` transposes and delegates to `Of`, and on the transpose the elimination finds nothing above its significance cut. So the user is handed only the column half --- the half `S-36` exists to warn against trusting alone. | Open, and pinned by `TheRowDirectionIsNotReportedOnACircuitThatHasOne`, which asserts the *absence* on purpose so the gap cannot be forgotten; flip it to `Contains` when this closes. The likely cause is that `Of`'s cut is calibrated for columns, whose scales are the unknowns' (pressures in Pa beside positions in 0..1), while rows carry residual scales instead --- but that is a guess and the measurement to settle it is to compare the two eliminations' pivots directly. **A diagnostic that is silent when it has something to say is worse than one that is merely incomplete**, because its silence reads as evidence. |
| S-39 | [`23`](../20-core-domain/23-topology-and-graph.md), [`36`](36-numerics-and-convergence.md) | **A pressure datum written on a junction silently turns a closed circuit into an open one, and the circuit is then deficient by exactly one** | `HasUnknownFlux` is true when a node *carries a mass balance* and states a pressure. For a `supply` or a `return` that is right --- mass genuinely crosses there. For a plain interior `node p=250` sitting on a three-way junction it is not: nothing enters, the circuit is closed, and its mass balances still sum to an identity. But the partition now looks open, so `EquationLayout.Redundancies` drops **no** balance --- measured, `0 balances dropped` on a 46-unknown header --- and no external-flux unknown appears to take up the slack either. The row null direction says so exactly: `NM_AHU + N1 + N3 mass balance`, every weight 1, same sign, which is the global conservation identity and nothing else. **Measured decisively**: the same fixture with the datum moved from the junction `N1` to the two-connection node `N2` stops being `Singular` at all. The rule is deliberate --- `WellPosedness` documents the carries-a-balance test in a comment written against it --- so this is a decision to revisit, not an oversight. | Open. The fix is one predicate: an *interior* node never admits unknown flux however it is annotated, so `HasUnknownFlux` should test `Boundary is not BoundaryRole.Interior` rather than a stated pressure. `m2-cooling-loop` is unaffected (its two stated pressures are a `supply` and a `return`), and the committed `m2-distribution-header` writes exactly the offending form. Because it changes what `node p=` *means* --- boundary condition, or datum --- it needs a `D-` entry beside `D-64` rather than an edit. |
| S-38 | [`23`](../20-core-domain/23-topology-and-graph.md), [`24`](../20-core-domain/24-auto-sizing.md), [`32`](32-steady-state-newton.md) | **Two consumers on a header have one collective freedom too many, and one consumer does not** | With the header differential unconstrained, raising both secondary pump heads and closing both three-way valves together changes nothing observable. The row direction names `PU_RAD.head + PU_AHU.head - 0.12 x TV_RAD.position - 0.12 x TV_AHU.position`, symmetric across the two consumers. **It survives away from the seed** --- measured at jitter 0.01, 0.05, 0.2 and 0.5, so it is structural rather than an `S-25`/`S-35` seed artefact. **One consumer is not deficient**: the single-consumer fixture, once `S-39`'s datum is moved off its junction, terminates `NonFinite` with the rank intact. `m2-cooling-loop` has the same subcircuit shape and is fine because `supply p=300` / `return p=280` fixes the drop its valve works against. | Open, and `M2a`'s remaining blocker. Stating a header differential removes it (measured: `Singular` becomes `NonFinite`) but does not make the circuit converge at any value from 3 to 100 kPa, so it is necessary and not sufficient. This is `S-37` one level down --- there two pumps share a head, here N consumers share a differential --- and both are resolved in practice by stating what the branch works against. **The first version of this entry led with the `FS3009` reading and called it a pump-versus-valve ambiguity within one secondary.** That is the column null direction, which `S-36` exists to warn against, and it was wrong twice over: the deficiency needs two consumers, and the row direction names something else. |
