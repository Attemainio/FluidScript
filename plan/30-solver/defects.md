...
| S-14a | [`22`](../20-core-domain/22-component-model.md), [`23`](../20-core-domain/23-topology-and-graph.md) | **A three-way valve wired as a two-way still writes a Kv law for the port nothing is connected to** | `27`'s registry makes port `c` optional and `docs/functions/three-way-valve.md` says in as many words that "a three-way valve used as a two-way leaves it open" — but `ThreeWayValve` declares three equations unconditionally, including `Kv law, a-c`, and its residual reads `context.Ports[2]` and `context.Flows[2]` for a port with no node behind it. `Relations()` is right and the component is wrong: it gives the valve `ends − 1` = 1 relation, and the valve declares 2. **The consequence is not cosmetic** — `m2-distribution-header` and `m1-syntax-tour` each hold two such valves, so the counting table reports both circuits square while the system they would assemble is over-specified by two. Found by comparing the two counts (`S-9`), not by anything failing. The fix is a design decision with a diagram consequence and is recorded for the user: see the options below. |
