...
| S-14 | [`23`](../20-core-domain/23-topology-and-graph.md), [`31`](31-solver-architecture.md) | **The declared pressure rows and `Relations()` disagree on three of the seven samples** | Measured while reconciling the assembler against the counting table. Declared `Pressure` + `ComponentConstraint` rows plus the ideal node-to-node links, against `CountingTable.PressureRelations`: `m1-syntax-tour` 21 against 19, `m2-distribution-header` 13 against 11, `m2-substation` 7 against 8. The other four agree exactly. Two deltas are in the components' favour and one in the table's, so it is not a single missing rule. `Relations()` gives a junction element that is not a node `ends − 1` rows where the component declares one per port pair, which is the first place to look. **The equation layout waits on this**; the unknown layout does not and shipped. |
