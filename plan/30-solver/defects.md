...
| S-11 | [`22`](../20-core-domain/22-component-model.md), [`23`](../20-core-domain/23-topology-and-graph.md), [`31`](31-solver-architecture.md) | **The rows the components declare and the rows the counting table counts disagree, by exactly one per heat exchanger** | Found by `P3.6a` doing what `S-9` asks — comparing the two counts before writing an assembler that would have had to pick one. On the cooling loop the table says 20 equations and the declarations plus the assembly's own boundary rows come to 21. The extra row is `HeatExchanger`'s duty, `Q = ṁ(h_out − h_in)`: `WellPosedness.Relations` counts a crossed component once, as a *pressure* relation, and `EnergyBalances` is `Nodes.Length` unconditionally, so nothing counts the duty. It is not a miscount but a contradiction about ownership — the node downstream of the exchanger writes `h_own = h_arriving`, and the exchanger writes `h_out − h_in = Q/ṁ`, and both cannot hold. Every sample shows it: the excess equals the number of non-node components declaring an energy row, 1 to 5 across the corpus. **The assembler cannot be written until this is settled**, because the disputed row is one it would have to emit or drop. |
