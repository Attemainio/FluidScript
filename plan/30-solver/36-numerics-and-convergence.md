...
| `jacobian.valley_tol` | 1e-3 | Smallest pivot / largest, full pivoting, at the converged answer | Below this the answer is one point on a valley and the direction is read (`FS3016`, `S-37`). Measured 2026-09-22: unique answers run 8e-3 and above, the series headers 3e-5 and below; one single-pump circuit with a tiny valve sits at 6e-6 and the direction test keeps it silent |
...
| `transient.energy_drift_tol` | 1e-2 | Relative, over a run | 1 % triggers `FS3106`; the numerator is `33`'s drift formula over the differential states only |
| `transient.settle_tol` | 1e-4 | Scaled change of any differential state per frame interval | Same scale as `local_error_tol`: a state that moves less than the integrator can resolve has stopped |
| `transient.settle_frames` | 10 | Consecutive frame intervals under `settle_tol` | Ten seconds at the default interval; shorter than the loop residence time and long enough that a slow drift is not read as rest (`FS3104`) |
| `valve.dp_regularization` | 100 Pa | Below this, the √ law is blended | See below — the blend is quadratic, not linear |
| `seed.valve_excursion` | 1.5e4 Pa | The magnitude of any port offset a valve's own law gives the seed's pressure walk | The seed reads each law at a flow it invented, and a nearly-shut leg at that flow asks for megapascals (the header's `TV_AHU` bypass wanted −6.8 MPa). Only a valve reaches the clamp; every other component writes a pressure row. The value is the top of ESBE's published 3–15 kPa selection band (`three_way.dp_max`): a valve wanting more than it would ever be selected for is a valve whose seeded flow is wrong, not a drop the circuit contains. Measured on the corpus and not monotone (`S-47`): 10 and 15 kPa green throughout; 40 kPa stalls `s55-main-valve`; 100 kPa breaks the three `s56` switched-off consumers but not `s55`; 300 kPa breaks all four; the largest pump's shut-off head, the circuit-derived bound the row proposed, stalls `s56-ahu-off-main`. Healthy loops lose one or two first-pass iterations to the cap. Sizing reads the same laws at solved flows and is not bounded |
