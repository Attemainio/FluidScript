...
|---|---|---|---|---|---|---|
| A-6 | 2026-09-18 | 2026-09-22 | medium | [`42`](42-rest-contract.md), [`26`](../20-core-domain/26-model-contract.md), [`54`](../50-frontend/54-interaction-and-writeback.md) | **The wire has no unit factors and no pipe velocity, so two hovers `52` and `54` specify cannot be drawn** | Two additions, both where the row said. `metadata.dimensions[].conversions` carries every unit symbol's factor and offset (`si = value × factor + offset`, `13`'s affine form -- 273.15 for °C, −101325 for an absolute pressure spelling) in the order of `units`, and the editor's quantity hover (`quantityCard`) shows a literal in SI and in every other unit of its dimension from them; `MetadataTests` holds the order, the positivity of every factor and the kW and °C numbers. A pipe's state carries `velocity` and `re`, computed in `ModelContractBuilder.State` from the same mean density and viscosity the pipe's own residual used (`Pipe.EvaluateResiduals`), so the velocity reported is the one the pressure drop was computed at; `SolvedPort` gained the viscosity for it, so no property call is spent. Measured on the simple loop's return: 0.28 m/s and Re 7.6e3 through DN25 at 0.24 kg/s, the density backed out of the wire within 990--1000 kg/m³, Re within 10 % of ρvD/1.0e-3. The connection and component cards show both rows; `54` and `52` say so; contract `2.2`. Effort as estimated. |
