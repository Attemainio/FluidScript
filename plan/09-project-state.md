...
| P6 | M4 | 9 | **In progress** — P6.0, P6.1, P6.2 and `C-114` shipped 2026-09-22; P6.8a and P6.8b 2026-09-22/23 (the scenario language and its sizing pipeline), `C-121` 2026-09-23 (valve authority and turn-down across scenarios). Next: P6.9 (the source structure, `D-147`/`D-148`, S0–S3 shipped), then `C-120`, then P6.3; the live-curve half waits on `S-79` | — |
| P7 | M5 | 2 | Not started | — |
...
- **P6.9 next, the source structure** ([`71`](71-source-structure.md), `D-147`), before the
  transient, the user's call. S0–S3 shipped: the decision, the domain folders, one type per file, the concern partials. S4 is
  next — `ComponentBase` under the seven components, the one package that touches the hot path, so
  the solver-scale timing is measured before and after. Measured before starting: the whole restructure adds about 1 700 lines net (+3 %); the
