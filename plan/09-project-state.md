...
| P6 | M4 | 9 | **In progress** — P6.0, P6.1, P6.2 and `C-114` shipped 2026-09-22; P6.8a and P6.8b 2026-09-22/23 (the scenario language and its sizing pipeline), `C-121` 2026-09-23 (valve authority and turn-down across scenarios). Next: P6.9 (the source structure, `D-147`/`D-148`, S0–S4 shipped), then `C-120`, then P6.3; the live-curve half waits on `S-79` | — |
| P7 | M5 | 2 | Not started | — |
...
- **P6.9 next, the source structure** ([`71`](71-source-structure.md), `D-147`), before the
  transient, the user's call. S0–S4 shipped: the decision, the domain folders, one type per file, the concern partials, the
  component bases. S5 is next — `SizerBase<TComponent>` and the pipe-catalogue builder. Measured before starting: the whole restructure adds about 1 700 lines net (+3 %); the
