...
| F-21 | [`01`](01-vision-and-scope.md) | **The distribution header's plant side has no loop of its own, and that is what makes it unsolvable** | `HS1` and `PU_MAIN` sit directly in the header path, so the source's flow **is** the network's flow and the plant has no freedom left to regulate with. The consequence is not subtle and it is not only physical: the fixture states `HS1 power=54 out=60` *and* both loads' `power` and `out`, and 24 + 30 = 54 exactly --- so the plant's energy balance is already implied by the loads', and the circuit states the same energy fact twice. **Measured**: with the consumers written as real mixing circuits, the in-line plant is **rank deficient** (37 unknowns, 37 equations, one row implied --- `N3` and `N5` mass balances against the energy balances) while the same consumers on a plant that has its own loop are **full rank** at 44 unknowns. Not a solver defect: a plant nobody would build. | Open, and it is the missing half of `F-16`/`F-17`. Those record that each *consumer*'s recirculation leg is unconnected; this is that the *source* needs the same thing. The arrangement the user identified, and the one this should become: `R1 - N1 - HX1 - PU1 - TV3 - S1` with `TV3 - N1`, so the source runs its own flow and recirculates what the network does not draw. That recirculation flow is the degree of freedom the in-line form lacks --- it is what lets a source hold a supply temperature while the load varies, and a real plant puts a low-loss header, a buffer tank or an injection circuit exactly there. Changing the fixture changes its inference inventory, its tag ordinals and `01`'s published flows, which `05` and `ReferenceCircuitTests` assert, so it is a decision rather than an edit. **Blocks `m2-distribution-header`, an `M2a` exit criterion.** |
|---|---|---|---|
...
| F-17 | [`01`](01-vision-and-scope.md), [`05`](05-milestones-and-acceptance.md) | **The distribution header's recirculation leg is not connected**, so the fixture cannot produce the flows its own design point assumes | Each subcircuit is `PU_x - HE_x - TV_x`, open at both ends, and the third port of each three-way valve is left unconnected and terminated (`FS2202`). As drawn, each subcircuit is once-through: whatever the header supplies arrives at the exchanger, so `in=50` on a 60 °C header has nothing to mix with and the valve has nothing to divert. The corrected figures in `F-16` assume the mixing the valve is there for — `TV_AHU.b` wired back to `PU_AHU.in` — and the counting pass is square either way, because it promotes each `in` to its valve's `position` without checking that the position can move anything. Left open because wiring it changes the fixture's inference inventory and its tag ordinals, which `05` and `ReferenceCircuitTests` both assert, and the flows it would then produce cannot be checked until a solver exists. |
| F-20 | [`03`](03-repository-layout.md) | **`global.json` was in the repository, load-bearing, and named by no plan document** | It is not in `03`'s target tree and has no subsection under Build configuration, although CI resolves its SDK through it (`setup-dotnet` with `global-json-file`) and its `rollForward` decides whether two installed SDKs can both claim the repository. That second consequence is the one nothing anywhere recorded: with `latestFeature` on a machine holding 10.0.111 and 10.0.301, the Roslyn tooling registered one band and the shell's `dotnet` resolved the other, both restored into one `ArtifactsPath`, and whichever ran last left an assets cache the other could not open — surfacing as a workspace that loaded with missing references and answered as though nothing were wrong. Narrowed to `latestPatch`, which names one band, and `03` now documents the file and the trade. Open only because the general form is unaddressed: **any tool that loads these projects outside `dotnet build` chooses its own SDK**, and nothing in `plan/` says that a repository's SDK selection is part of its contract with its tooling rather than a local convenience. |
