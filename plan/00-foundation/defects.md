...
| F-15 | [`01`](01-vision-and-scope.md) | **The simple-loop reference circuit has no steady solution**, and the counting check catches it | `HE1 heat_exchanger power=30 in=20 out=50` on a closed adiabatic ring: continuity around the loop forces the enthalpy arriving at `HE1` to equal the enthalpy leaving it, so `in=20` with `out=50` cannot be satisfied by any valve, pump or pipe — and there is no sink anywhere in the script to remove the 30 kW. `01` says "the flow is fixed by the energy balance at 0.2392 kg/s", which is true of the exchanger's own equation in isolation and leaves the loop's energy unbalanced. Well-posedness reports it over-specified by one, naming `HE1.in`. The minimal fix is a load on the return leg; making it is a change to a reference circuit several documents quote, so it is the user's call rather than this package's. |
| F-16 | [`01`](01-vision-and-scope.md) | **The distribution header states three mixed inlet temperatures with two mixing valves, and a return temperature two 30 °C loads cannot produce** | `HS1 in=40` is the temperature arriving back at the plant-side source. Both consumer circuits state `out=30`, so the mixed return is 30 °C and not 40 °C; and structurally, three `in=` demands need three things that can move an inlet temperature while the script has two three-way valves. Well-posedness reports it over-specified by one, naming `HE_RAD.in`. Like `F-15` this is a defect in the reference circuit rather than in the check — and it is the more interesting of the two, because both individually reasonable numbers are wrong only in combination. |
