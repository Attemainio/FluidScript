...
| `R-52` | An **equipment list** enumerates every device with its design-point values, grouped by circuit in derived tag order, rendered as a side panel and exported as CSV. Derived columns are projected from the model contract; the equipment model, notes, and electrical supply data are declared in the script and edited from the panel through write-back. Post-v1, and gated by [`73-equipment-list`](../70-future/73-equipment-list.md). |
