...
| Layout solve | A component of the keystroke-to-diagnostic gate since `D-103`: placement and routing of the 200-component reference model complete within 30 ms p95 on the reference environment, measured inside `ModelContractBuilder`, and the layout is byte-identical across two runs of the same source (`D-72`). Recorded by P5.1d-1: 2026-09-15, 35--40 ms best-of-five on the 200-component header in a Debug build with every graph element in a cell; 2026-09-16, **24 ms** after `D-105` took the pipes and two-port nodes out of the grid (fewer cells, fewer runs to route) and the U planner replaced the columns, so the target is met with a Debug build and 6 ms of headroom; **21 ms** later the same day with the `OrthogonalRouter` (a Hanan grid per run, not per cell), so a router did not cost what P5.1d-1 feared; `ModelContractBuilder.Build` with the layout is 34--46 ms warm; **56 ms** on 2026-09-17 once the ladder engine (`28`, step 8) drew the header through its ring rules instead of the fallback grid, the branch search and the clearance tests unprofiled (`C-92`). **48 ms** in a Release build the same evening, and 27 ms for the 400-component header (`C-92`: the cost is not proportional to the picture). **Met since the composed engine replaced it** (P6.10 R6,
2026-09-24): 4.4 ms median, **12.7--15.7 ms p95** over four runs of 200 after warm-up in a Release build, 20.4 ms
best-of-five in Debug. The rest of the pipeline on the same build and model, best of five: parse 0.5 ms, bind 2.8 ms, lower and prepare 14 ms, layout hints 1 ms; the circuit solve is 84 ms with constant-property water and 290--490 ms with the real backend, and is not part of this gate. `LayoutTimingTests` asserts under 250 ms so the suite cannot regress past the compile gate unnoticed. |
